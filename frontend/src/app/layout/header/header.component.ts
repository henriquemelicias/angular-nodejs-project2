...
export class HeaderComponent implements OnInit {
    ...
    this._sessionUserSubscription = UserService.getSessionUser$().subscribe(
      {
        ...
        error: error => {
          LoggerService.error( JSON.stringify( error ), LoggerService.setCaller( "HeaderComponent", this.ngOnInit ) );
        }
      }
    );
  ...
  updateNavItems() {
    this.navItems = [
      ...
      { link: '/contact', routerLink: 'contact', title: 'Contact', isVisible: true },
      ...
      { link: '/task', routerLink: 'task', title: 'Tasks', isVisible: this.isLoggedIn },
    ];
  }
  ...
  openModal() {
    this.modalRef = this.modal.open( ModalComponent, { data: { title: 'Logout', content: 'Do you want to logout?' } } );
