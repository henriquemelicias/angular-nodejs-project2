import { ClusterNode, DagreSettings, Graph, Layout, Orientation, Node, Edge } from "@swimlane/ngx-graph";
import * as dagre from 'dagre';
import { PseudoRandom } from "webcola";

export class DagreClusterCustomLayout implements Layout {
    ...
    run(graph: Graph): Graph {
        ...
        const dagreToOutput = ( node: { id: string | number; }) => {
    ...
    createDagreGraph(graph: Graph): any {
        ...
        this.dagreEdges = graph.edges.map(l => {
            ...
            if (!newLink.id) {
                newLink.id = new PseudoRandom().getNext();
            }
            return newLink;
        });
